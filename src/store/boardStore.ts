...
interface BoardState extends Board {
  addTask: (columnId: string, title: string, description?: string, priority?: Task['priority']) => void;
      ...
      moveTask: (taskId, fromColumnId, toColumnId, newIndex) => {
        set((state) => {
          ...
          return {
            columns: {
              ...state.columns,
              [fromColumnId]: { ...fromColumn, taskIds: fromColumnId === toColumnId ? toTaskIds : fromTaskIds },
